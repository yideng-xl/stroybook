...
public class StoryController {
    ...
    public List<Story> getStories(@RequestParam(required = false) String keyword,
        ...
        log.info("getStories called by user: {}, status: {}, keyword: {}, mine: {}", userId, status, keyword, mine);

        List<Story> stories;
        if (mine && userId != null) {
            stories = storyService.getUserStories(userId);
    ...
    public ResponseEntity<?> getStoryContent(@PathVariable String id,
            ...
            if (!hasReadToday) {
                ...
                if (count >= 2) {
                    return ResponseEntity.status(HttpStatus.FORBIDDEN)
                            .body("Daily limit reached (2 stories). Please login.");
                }
                // Record new reading
    ...
    @PostMapping("/{id}/redub")
    public ResponseEntity<Void> redubStory(@PathVariable String id, @RequestParam Long voiceId) {
        Authentication auth = SecurityContextHolder.getContext().getAuthentication();
        String userId = auth.getName();
        storyService.redubStory(id, voiceId, userId);
