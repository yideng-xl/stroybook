...
public class StoryController {
    ...
    public ResponseEntity<GenerateStoryResponse> generateStory(@Valid @RequestBody GenerateStoryRequest request) {
        ...
        String storyId = storyService.initiateStoryGeneration(userId, request.getPrompt(), request.getStyle(), request.getVoiceId());
    ...
    public ResponseEntity<Void> n8nCallback(@Valid @RequestBody StoryCallbackRequest request) {
        storyService.handleN8NCallback(request.getStoryId(), request.getStatus(), request.getErrorMessage(), request.getType());
